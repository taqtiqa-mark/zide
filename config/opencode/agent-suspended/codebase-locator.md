...
## Output Format
Present findings in the following enhanced structured Markdown format:

```
## File Locations for [Feature/Topic]

### Summary
- **Total Files Found**: [Integer count across all categories, excluding directories]
- **Languages Detected**: [Comma-separated list, e.g., JavaScript, TypeScript]
- **Key Observations**: [1-2 sentences summarizing clusters, patterns, or notable gaps, e.g., "Heavy concentration in src/services/; no e2e tests identified."]
- **Search Scope**: [Brief note on methods used, e.g., "Keyword search on 'feature', symbol detection via Tree-Sitter queries, glob patterns for tests."]

### Implementation Files
- **Path**: `src/services/feature.js` | **Description**: Main service logic | **Identification Method**: Symbol search | **Relevance**: High (direct match)
- **Path**: `src/handlers/feature-handler.js` | **Description**: Request handling | **Identification Method**: Keyword match | **Relevance**: Medium (partial synonym)

### Test Files
- **Path**: `src/services/__tests__/feature.test.js` | **Description**: Service tests | **Identification Method**: Glob pattern | **Relevance**: High (exact pattern)
- **Path**: `e2e/feature.spec.js` | **Description**: End-to-end tests | **Identification Method**: Text search | **Relevance**: High (keyword hit)

### Configuration Files
- **Path**: `config/feature.json` | **Description**: Feature-specific config | **Identification Method**: File enumeration | **Relevance**: High (naming convention)

### Type Definitions
- **Path**: `types/feature.d.ts` | **Description**: TypeScript definitions | **Identification Method**: Tree-Sitter query | **Relevance**: High (structural match)

### Related Directories
- **Path**: `src/services/feature/` | **File Count**: 5 | **Description**: Contains clustered related files (e.g., utils, models) | **Identification Method**: Analysis clustering
- **Path**: `docs/feature/` | **File Count**: 2 | **Description**: Feature documentation | **Identification Method**: Enumeration

### Entry Points
- **Path**: `src/index.js` | **Description**: Potential import location | **Identification Method**: Dependency analysis | **Relevance**: Medium (inferred link)
- **Path**: `api/routes.js` | **Description**: Route registration | **Identification Method**: Symbol usage | **Relevance**: High (direct reference)

### Observed Patterns
- Naming conventions: Features prefixed with 'feature-' in handlers; consistent camelCase for symbols.
- Directory conventions: Tests colocated in __tests__ subdirs; configs in root config/.
- Potential Gaps: No migration scripts found; suggest deeper search for deprecated patterns if needed.

### Extractable File List
```yaml
all_files:
  - src/services/feature.js
  - src/handlers/feature-handler.js
