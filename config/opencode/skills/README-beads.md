# OpenCode Skill for Beads

A comprehensive OpenCode skill that teaches OpenCode how to use beads effectively for issue tracking in multi-session coding workflows.

## What is This?

This is a [OpenCode](https://opencode.ai) skill - a markdown-based instruction set that teaches OpenCode AI how to use beads. While beads provides MCP tools for basic operations, this skill complements it by teaching the **philosophy and patterns** of effective beads usage.
...
- Compaction survival patterns (critical for OpenCode context limits)
...
2. Have [OpenCode](https://opencode.ai) installed
...
# Create a symlink in your OpenCode skills directory
ln -s "$(pwd)" ~/.opencode/skills/beads
```
...
mkdir -p ~/.opencode/skills/bd-issue-tracking

# Copy the skill files
cp -r beads/examples/claude-code-skill/* ~/.opencode/skills/beads/
```

### Verify Installation

Restart OpenCode, then in a new session, ask:

```
Do you have the bd skill installed?
```

OpenCode should confirm it has access to the bd skill and can help with beads issue tracking.

## How It Works

OpenCode automatically loads skills from `~/.opencode/skills/`. When this skill is installed:

1. OpenCode gets the core workflow from `SKILL.md` immediately
2. OpenCode can read reference docs when it needs detailed information
...
You can also explicitly ask OpenCode to use beads:
...
This skill complements the [beads plugin](../../.opencode-plugin/):

- **Plugin** (`.opencode-plugin/`): Provides slash commands (`/bd-create`, `/bd-ready`) and MCP tools for basic operations
- **Skill** (this directory): Teaches OpenCode the patterns, philosophy, and decision-making for effective beads usage
...
This skill teaches OpenCode to use the bd CLI directly (via Bash commands like `bd ready`, `bd create`, etc.) rather than relying on MCP tools. This approach has several benefits:
...
The MCP server is excellent for interactive use, but for autonomous agent workflows where context efficiency matters, direct CLI usage is more practical. The skill provides the guidance OpenCode needs to use the CLI effectively.
