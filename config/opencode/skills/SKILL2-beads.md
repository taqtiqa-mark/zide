...
**bd solves:**
- Context loss from compaction
- "What was I doing?" after breaks
- Dependency hell in complex projects
- Tracking side quests without losing main thread

**Key principles:**
- **Persistent memory**: Database + git survives compaction
- **Automatic readiness**: Dependencies compute "what's next"
- **Discovery tracking**: Link issues to preserve context
- **Minimal overhead**: Simple CLI, single file db
...
- For essential daily workflows (session start, discovery, epic planning, compaction, etc.), see [references/WORKFLOWS_CORE.md](references/WORKFLOWS_CORE.md)
